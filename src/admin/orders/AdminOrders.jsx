...
// import axiosInstance from '../../utils/axiosInstance'; // JWT-enabled axios
...
//     try {
//       const res = await axiosInstance.get('adminpanel/orders/');
//       setOrders(res.data);
//       setLoading(false);
//     } catch (err) {
...
//     try {
//       // Adjust backend to allow PATCH update for order status
//       await axiosInstance.patch(`adminpanel/orders/${id}/`, { status: newStatus });
...
//       try {
//         await axiosInstance.delete(`adminpanel/orders/${id}/`);
              ...
              currentOrders.map((order) => (
                <React.Fragment key={order.id}>
                  <tr className="border-t hover:bg-pink-50 transition">
                    <td className="px-4 py-3">{order.id}</td>
                    <td className="px-4 py-3">{order.user?.username || 'N/A'}</td>
                    <td className="px-4 py-3">{new Date(order.date).toLocaleString() || 'N/A'}</td>
                    <td className="px-4 py-3 font-semibold text-pink-700">₹{order.total}</td>
                    <td className="px-4 py-3">
                      <select
                        value={order.status}
                    ...
                    <td className="px-4 py-3 flex gap-2 items-center">
                      <button
                        onClick={() => setViewDetails(viewDetails === order.id ? null : order.id)}
                        className="flex items-center gap-1 bg-pink-100 hover:bg-pink-200 text-pink-700 px-3 py-1 text-sm rounded-full transition"
                      >
                        <FaEye className="text-xs" /> View
                      </button>
