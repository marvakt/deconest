...
//       setIsLoading(true);
//       try {
//         // ✅ Removed leading slash to prevent double-slash issue
    ...
    <section className="py-12 sm:py-16 md:py-20 lg:py-24 px-4 sm:px-6 bg-white relative">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-10">
          <h2 className="text-4xl font-extralight mb-3">Trending Now</h2>
          <div className="w-16 h-px bg-gray-800 mx-auto mb-4"></div>
          <p className="text-gray-600 font-light text-lg opacity-80">
            Most loved pieces this season
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 md:gap-10 lg:gap-12">
          {trendingProducts.map((item, index) => {
            const isInWishlist = wishlist.some((w) => w.product?.id === item.id);

            return (
              <div
                key={item.id}
                className="group relative bg-white rounded-2xl overflow-hidden shadow-xl border border-gray-100"
              >
                <div className="relative overflow-hidden">
                  <img
                    src={item.image}
                  ...
                  <button
                    onClick={() => handleWishlistClick(item)}
                    className="absolute top-3 right-3 z-20 w-10 h-10 bg-white/80 backdrop-blur-md rounded-full shadow-lg flex items-center justify-center transition-all duration-300 hover:scale-110"
                    title={isInWishlist ? "Remove from Wishlist" : "Add to Wishlist"}
                  >
                    {isInWishlist ? (
                ...
                <div className="p-4 sm:p-6">
                  <h3 className="text-lg font-light mb-2 text-gray-900">{item.title}</h3>
                  <p className="text-xl font-light text-gray-800 mb-4">₹{item.price}</p>

                  <button
                    onClick={() => handleAddToCart(item)}
                    className="w-full bg-gray-900 text-white px-4 py-3 rounded-xl font-medium transition-all duration-300 hover:bg-gray-800"
                  >
                    Add to Cart
                  </button>
                </div>
